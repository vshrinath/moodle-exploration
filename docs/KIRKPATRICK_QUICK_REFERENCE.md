...
## Overview
The Kirkpatrick Model evaluates training effectiveness across four levels: Reaction, Learning, Behavior, and Results. This guide maps each level to the Moodle tools and plugins available in the SCEH LMS.
...
### Configuration
```bash
# Configure Kirkpatrick plugins (run inside container)
docker compose -f docker-compose.moodlehq.yml exec moodle \
  php /var/www/html/public/scripts/config/configure_kirkpatrick_plugins.php

# Configure individual levels
docker compose -f docker-compose.moodlehq.yml exec moodle \
  php /var/www/html/public/scripts/config/configure_kirkpatrick_level1_reaction.php
```

### Related Plugins
- `local_kirkpatrick_dashboard` — Unified dashboard (Levels 1-3)
- `local_kirkpatrick_level4` — ROI/external data integration
- `local_sceh_rules` — Rules engine for competency automation
...
### Scripts Location
- Configuration: `scripts/config/configure_kirkpatrick_*.php`
- Dashboard plugin: `local_kirkpatrick_dashboard/`
- Level 4 plugin: `local_kirkpatrick_level4/`
