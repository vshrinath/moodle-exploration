...
```bash
docker exec moodle-exploration-moodle-1 php -r 'define("CLI_SCRIPT", true); require("/bitnami/moodle/config.php"); global $DB; $users=["mock.sysadmin","mock.programowner","mock.trainer","mock.learner"]; foreach($users as $u){ $user=$DB->get_record("user",["username"=>$u,"deleted"=>0],"id,username",IGNORE_MISSING); if(!$user){ echo "MISSING\t$u\n"; continue; } echo "USER\t{$user->username}\n"; $sql="SELECT r.shortname FROM {role_assignments} ra JOIN {role} r ON r.id=ra.roleid WHERE ra.userid=:uid AND ra.contextid=1 ORDER BY r.shortname"; $roles=$DB->get_records_sql($sql,["uid"=>$user->id]); foreach($roles as $r){ echo "  ROLE\t{$r->shortname}\n"; } }'
```
