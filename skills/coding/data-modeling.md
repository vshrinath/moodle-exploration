...
class Article(models.Model):
    ...
    tags = models.ManyToManyField(Tag, through='Authorship')           # M2M with extra fields
...
views   = models.IntegerField(default=0)                         # Not CharField
active  = models.BooleanField(default=True)
meta    = models.JSONField(default=dict)                         # Flexible data
...
class Meta:
    constraints = [
        ...
        UniqueConstraint(fields=['article', 'author'], name='unique_article_author'),
    ]
...
Article.objects.select_related('author')    # FK/OneToOne — SQL JOIN
Article.objects.prefetch_related('tags')    # M2M/reverse FK — separate query
...
const articles = await prisma.article.findMany({
  include: { author: true, tags: true }   // Prevents N+1
});
